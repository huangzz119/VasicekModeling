...
for i in np.arange(len(alpha_all)):
    ...
    def alpha_to_var(x):
        ...
        return untp - (1 - alpha)
    var = fsolve(alpha_to_var, np.array([1247]))
    ...
    def var_to_es533(var):
        ...
        for i in np.arange(len(cpd)):
            ...
            def cgf_fir_(t):
                ...
                sed_cgf_num = (1 - pd) * w**2 * pd * np.exp(w * t)
                sed_cgf_den = np.square(1 - pd + pd * np.exp(w * t))
                sed_cgf = sum((sed_cgf_num / sed_cgf_den))
            ...
            sed_num = (1 - pd) * w**2 * pd * np.exp(w * t)
            ...
            if sed_cgf_ > 0 and 2 * (var[0] * t[0] - cgf_) > 0:
...
plt.title("Example 2",fontsize = 40)
plt.savefig("2allvar" + ".png")
