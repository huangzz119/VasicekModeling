...
result = pan.DataFrame({"tail probability":y,"alpha":alpha_all,"VaR(MC)":var_mean,"VaR(SP)":varsp})


############################################ For Vesicek #########################


varv = vesicek_method(y, unpd, rho, ead, lgd)
...
#plt.ylim((0.0001, 0.1))
#plt.xlim((0.05, 0.225))
plt.plot(varsp[3:], y[3:],'o-', label='Saddlepoint Method')
#plt.plot(varv,y, label='Vasicek Method' )
plt.plot(var_mean[3:], y[3:], label = "Monte Carlo Simulation")
#plt.plot(x_loss_percent_Vesicek,y_probability_Vesicek)

plt.xlabel('VaR', fontsize = 30)
plt.ylabel('Tail probability', fontsize = 30)
plt.grid(which='both', linestyle=':', linewidth=1)
plt.legend(fontsize=30)
plt.title("Example 4",fontsize = 40)
plt.savefig("4allvar" + ".png")
plt.show()
...
#plt.ylim((0.0001, 0.1))
#plt.xlim((0.05, 0.225))
plt.plot(essp[3:], y[3:],'o-', label='Saddlepoint Method')
#plt.plot(varv,y, label='Vasicek Method' )
plt.plot(es_mean[3:], y[3:], label = "Monte Carlo Simulation")
#plt.plot(x_loss_percent_Vesicek,y_probability_Vesicek)
